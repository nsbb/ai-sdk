...
{
    vnn_ReleaseOutputNpu( graph, TRUE );
...
{
    return vnn_PostProcessOutputNpu( graph );
}
    ...
    )
{
    ...
    if(vnn_UseImagePreprocessNode())
    {
        return vnn_PreProcessOutputNpu_ImageProcess(graph, inputs, input_num);
    }
    */
    return vnn_PreProcessOutputNpu( graph, inputs, input_num );
}
    ...
    )
{
    ...
    tmsStart = get_perf_count();
    graph = vnn_CreateOutputNpu( data_file_name, NULL,
