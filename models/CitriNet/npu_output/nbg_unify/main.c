...
{
    vnn_ReleaseCitrinetNpu( graph, TRUE );
...
{
    return vnn_PostProcessCitrinetNpu( graph );
}
    ...
    )
{
    ...
    if(vnn_UseImagePreprocessNode())
    {
        return vnn_PreProcessCitrinetNpu_ImageProcess(graph, inputs, input_num);
    }
    */
    return vnn_PreProcessCitrinetNpu( graph, inputs, input_num );
}
    ...
    )
{
    ...
    tmsStart = get_perf_count();
    graph = vnn_CreateCitrinetNpu( data_file_name, NULL,
