...
{
    vnn_ReleaseWav2vec220sPrj( graph, TRUE );
...
{
    return vnn_PostProcessWav2vec220sPrj( graph );
}
    ...
    )
{
    ...
    if(vnn_UseImagePreprocessNode())
    {
        return vnn_PreProcessWav2vec220sPrj_ImageProcess(graph, inputs, input_num);
    }
    */
    return vnn_PreProcessWav2vec220sPrj( graph, inputs, input_num );
}
    ...
    )
{
    ...
    tmsStart = get_perf_count();
    graph = vnn_CreateWav2vec220sPrj( data_file_name, NULL,
