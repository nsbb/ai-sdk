...
INPUT_DAT="${WORK_DIR}/input_0.dat"
...
# Push input
$WIN_ADB push "$INPUT_DAT" "$DEVICE_DIR/input_0.dat" > /dev/null 2>&1
...
try:
    ...
    out_meta = [v for v in meta.values() if 'output' in str(v).lower() or 'out' in str(v).lower()]
    # Default scale/zp
