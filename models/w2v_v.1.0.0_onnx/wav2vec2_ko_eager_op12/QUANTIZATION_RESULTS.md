...
- [ ] **int16 양자화 시도** — T527 NPU는 int16(dynamic_fixed_point)을 지원함 (zipformer int16 디바이스 실행 확인). uint8 CER ~133% 문제를 해결할 수 있는 대안. 이전 wav2vec2 int16 "HANG" 보고는 VSIMULATOR_CONFIG 미설정(CID 불일치)이 원인일 가능성.
