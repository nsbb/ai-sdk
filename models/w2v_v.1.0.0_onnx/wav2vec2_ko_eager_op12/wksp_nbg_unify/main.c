...
{
    vnn_ReleaseWav2vec2KoEagerOp123s( graph, TRUE );
...
{
    return vnn_PostProcessWav2vec2KoEagerOp123s( graph );
}
    ...
    )
{
    ...
    if(vnn_UseImagePreprocessNode())
    {
        return vnn_PreProcessWav2vec2KoEagerOp123s_ImageProcess(graph, inputs, input_num);
    }
    */
    return vnn_PreProcessWav2vec2KoEagerOp123s( graph, inputs, input_num );
}
    ...
    )
{
    ...
    tmsStart = get_perf_count();
    graph = vnn_CreateWav2vec2KoEagerOp123s( data_file_name, NULL,
