...
static int detect_yolact(const cv::Mat& bgr, float **outputs, std::vector<Object>& objects)
{
    ...
    float *location = outputs[0];
    float *confidence = outputs[1];
    float *mask = outputs[2];
    float *maskmaps = outputs[4];

    int num_class = 81;		
    const int num_priors = 19248;	
    float priorbox[num_priors*4] = {0.0f};
    ...
    const float confidence_thresh = 0.40f;
    ...
    {
        const float* conf = &confidence[81*i];
        const float* loc = &location[4*i];
        const float* pb = &priorbox[4*i];
        const float* maskdata = &mask[32*i];
        ...
        obj.label = label;
        obj.prob = score;
        obj.maskdata = std::vector<float>(maskdata, maskdata + 32/*mask.w*/);
    ...
    for (int i = 0; i < objects.size(); i++)
    {
        ...
        cv::Mat mask(138/*maskmaps.h*/, 138/*maskmaps.w*/, CV_32FC1);
        {
            mask = cv::Scalar(0.f);

            for (int p = 0; p < 32/*maskmaps.c*/; p++)
            {
                const float* maskmap = maskmaps;
                ...
                {
                    int idx1 = 138*j;	//hw
                    int idx2 = 138*32*j;	//hwc

                    for (int t = 0; t < 138; t++)
                    {
                        mp[idx1 + t] += maskmap[idx2 + 32*t + p] * coeff;
                    }
                }
            }
        }
...
{
	cv::Mat m;
	cv::Mat temp(550, 550, CV_8UC3, (void *)pixels);
    cvtColor(temp, m, cv::COLOR_RGB2BGR);

	std::vector<Object> objects;
	detect_yolact(m, results, objects);
