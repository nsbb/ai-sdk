...
10. [Acuity Toolkit 버전 비교 (6.12 vs 6.21)](#acuity-toolkit-버전-비교-612-vs-621)
11. [환경 설정](#환경-설정)
...
| **Wav2Vec2 (5s)** | 영어 | HuggingFace → ONNX | `[1,80000]` INT8 | ✅ 추론성공 (87MB, 720ms) | CER 17.52% (6.12), [비교](docs/acuity_612_vs_621.md) |
...
모델 변환 SDK의 핵심 도구인 Acuity Toolkit은 두 가지 버전을 사용할 수 있습니다.
...
### 양자화 지원 및 T527 NPU 호환성

| 양자화기 | 6.12 | 6.21 | T527 NPU 실행 |
|---------|------|------|---------------|
| asymmetric_affine (uint8) | O | O | **동작** (하드웨어 가속) |
| perchannel_symmetric (PCQ int8) | O | O | **동작** (하드웨어 가속) |
| dynamic_fixed_point (int16) | O | O | HANG (물리적 리셋 필요) |
| bfloat16 / qbfloat16 | O | O | segfault / HANG |
| float16 (`--dtype float16` export) | X | **O** | 동작 (VX 셰이더 에뮬레이션, **25배 느림**) |
| e4m3 / e5m2 (FP8) | X | **O** | 미테스트 |

> **핵심**: T527 NPU는 **uint8/int8 MAC 유닛만** 하드웨어 탑재.
> fp16은 NB 생성·실행 가능하나 프로그래머블 셰이더로 소프트웨어 실행됨 (17.7초 vs uint8 0.7초).
> int16/bf16은 NB 생성 가능하나 디바이스에서 HANG 또는 segfault.
...
| Graph optimization | 보수적 | 공격적 (NB 크기↓, 정확도 하락 가능) |
| `--quantizer float16` | 지원 | 지원 (VX shader 코드 생성 → 컴파일 실패) |
| `--dtype float16` (export) | 미지원 | **지원** (fp16 NB 생성, 정확도 검증용) |

### 실측 비교 (Wav2Vec2 영어, LibriSpeech 50샘플)
...
- **신규 모델 변환**: 6.12 uint8로 먼저 시도. 정확도 부족시 6.21 PCQ 비교
- **fp16**: 양자화 열화 분석용으로만 사용 (17.7초/추론 → 실서비스 불가)
- **모델별 차이**: 6.12가 항상 좋은 것은 아님. 새 모델마다 양쪽 테스트 권장
...
```bash
WORK=/path/to/model/dir
VIVANTE57=~/VeriSilicon/VivanteIDE5.7.2
ACUITY612=~/acuity-toolkit-binary-6.12.0
...
docker run --rm -v $WORK:/work -v $VIVANTE57:/vivante57:ro -v $ACUITY612:/acuity612:ro \
  t527-npu:v1.2 bash -c "
    ...
    ./pegasus export ovxlib --pack-nbg-unify --optimize VIP9000NANOSI_PLUS_PID0X10000016 \
      ...
      --model /work/model.json --model-data /work/model.data \
      --model-quantize /work/model_uint8.quantize --dtype quantized \
      --output-path /work/wksp/
  "
```

> **주의**: `EXTRALFLAGS` rpath 설정 필수. `cd /acuity612/bin` 필수 (template 경로 참조).
...
docker run --rm -v $WORK:/work:rw ubuntu-npu:v1.8.11 bash -c "
    ...
    # 핵심: VivanteIDE 라이브러리를 /usr/lib에 심링크 (gen_nbg가 libvdtproxy.so 등을 찾기 위해)
    ...
    \$PEGASUS export ovxlib --with-input-meta inputmeta.yml \
      ...
      --model-quantize model_uint8.quantize --dtype quantized \
      --output-path /work/wksp/
  "
```

> **주의**: `--with-input-meta` 필수 (6.12에서는 불필요). libvdtproxy.so 심링크 필수.
